...
public partial class WorldObjectEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (worldObject.OverrideInitEnabled)
            this.DrawPropertySection(
                "Initial Enabled Overrides",
                ref showInitiallyEnabled,
                "OnInitialState",
                "OnInitiallyEnabled",
                "OnInitiallyDisabled"
            );
        ...
        if (worldObject.OverrideInitOperating)
            this.DrawPropertySection(
                "Initial Operating Overrides",
                ref showInitiallyEnabled,
                "OnInitialOperatingState",
        ...
        var defaultRedColorProp = serializedObject.FindProperty("defaultRedColor");
        if (defaultRedColorProp != null)
            EditorGUILayout.PropertyField(defaultRedColorProp);

        var defaultBlueColorProp = serializedObject.FindProperty("defaultBlueColor");
        if (defaultBlueColorProp != null)
            EditorGUILayout.PropertyField(defaultBlueColorProp);

        var defaultGreenColorProp = serializedObject.FindProperty("defaultGreenColor");
        if (defaultGreenColorProp != null)
            EditorGUILayout.PropertyField(defaultGreenColorProp);
    ...
    private void DrawDictionary(
        string title,
        ref string[] keys,
        Action<int> setArrayLengths,
        Action<int> drawArrayValueForIndex
    )
    {
    ...
    private void DrawArrayElement(string label, string arrayName, int index)
    {
        SerializedProperty property = serializedObject.FindProperty(
            string.Format("{0}.Array.data[{1}]", arrayName, index)
        );
        if (property != null)
